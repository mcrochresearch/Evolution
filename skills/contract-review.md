...
description: >
  Contract review and redline skill for in-house legal teams. Use when reviewing contracts
  against a negotiation playbook, identifying clause deviations, classifying risk severity
  (GREEN/YELLOW/RED), generating redline language, building objection responses, or creating
  negotiation priority tiers. Covers limitation of liability, indemnification, IP ownership,
  data protection/DPA, term and termination, governing law, and dispute resolution. Produces
  actionable redlines with rationale, fallback positions, and priority classification ready
  for counsel review. Use for LocalLoop vendor agreements, client MSAs, SaaS agreements,
  partnership contracts, and procurement deals.
metadata:
  author: localloop
  version: '1.0'
  category: legal
...
Contract review assistant for in-house legal teams. Analyzes contracts against a negotiation playbook, identifies deviations, classifies severity, and generates actionable redline suggestions.
...
Before reviewing any contract, check for a configured playbook in local settings. The playbook defines standard positions, acceptable ranges, and escalation triggers for each major clause type.

If no playbook is available:
- Offer to help create one
- If proceeding without a playbook, use widely-accepted commercial standards as baseline
...
- Scope: what triggers the obligation (IP infringement, data breach, bodily injury, breach of reps and warranties)
...
**Proposed redline**: "[specific alternative language — additions in bold, deletions struck through]"
...
**For vendor agreements** (SaaS tools, API providers, contractors):
- Focus on: data protection (who processes what), IP ownership of outputs, liability cap relative to contract value
- Key risk: AI-generated content ownership, API output rights (check vendor ToS vs. contract)

**For client MSAs** (LocalLoop → SMB clients):
- Drafting side — ensure limitation of liability caps your exposure at 3 months of fees
- Ensure IP ownership of marketing assets is clearly assigned to the client upon payment
- Include termination for convenience with 30-day notice (protects both sides)

**For INFERNO engagements** ($2K-5K/mo):
- At this contract value, a proper MSA + SOW is warranted
- Include deliverable acceptance criteria to avoid scope creep disputes
- Add non-solicitation (client can't hire your team directly for 12 months)
